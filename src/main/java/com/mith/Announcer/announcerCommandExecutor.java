  ...
  boolean onListCommand(CommandSender sender, Command command, String label, String[] args) {
    if (sender.hasPermission("announcer.moderate")) {
      if (args.length == 1 || args.length == 2) {
        ...
        sender.sendMessage(ChatColor.GREEN + String.format(" === Announcements [Page %d/%d] ===", new Object[] {page,
        ...
        for (int index = indexStart + 1; index <= indexStop; index++) {
          sender.sendMessage(String.format("%d - %s", index, chatColorHelper.colorize(
    ...
    if (sender.hasPermission("announcer.moderate")) {
      if (args.length > 1) {
        ...
        sender.sendMessage(String.format("%sPrefix is %s", ChatColor.LIGHT_PURPLE,
                chatColorHelper.colorize(this.plugin.getAnnouncementPrefix())));
      } 
